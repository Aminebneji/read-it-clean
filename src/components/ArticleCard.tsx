    ...
    return (
        ...
        >
            <Link href={`/articles/${article.id}`} className="block h-full group pt-20 pl-8">
                ...
                <Card className="relative overflow-visible border border-border/50 bg-card/60 backdrop-blur-sm hover:bg-card/90 transition-all duration-500 h-full flex flex-col shadow-sm group-hover:shadow-2xl rounded-[2rem]">
                    {article.image && (
                        <div
                            className="absolute -top-16 -left-8 w-52 h-56 md:w-64 md:h-64 z-10 shadow-2xl rounded-3xl overflow-hidden border-2 border-background pointer-events-none"
                            style={{
                        ...
                        >
                            ...
                            />
                            {/* gradient en light mode */}
                            <div className="absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t from-card/70 via-card/30 to-transparent dark:from-card/80 dark:via-card/20 dark:to-transparent z-20" />
                        </div>
                    )}
                    ...
                    <CardHeader className={`p-8 ${article.image ? 'pl-40 md:pl-52' : ''} pb-2 relative z-20`}>
                        <div className={`flex ${isLarge ? 'items-center gap-3' : 'flex-col gap-1.5'}`}>
                            <Badge
                                variant="outline"
                                className={`text-[10px] uppercase tracking-[0.1em] font-bold border-none px-2.5 py-1 ml-10 w-fit ${article.category === 'Classic'
                            ...
                            <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider opacity-60 ml-10 ">{formattedDate}</span>
                        </div>
                    </CardHeader>

                    <CardContent className={`p-8 ${article.image ? 'pt-8 md:pt-14' : 'pt-0'} flex-1 flex flex-col relative z-20`}>
                        <h2 className={`font-black text-card-foreground mb-6 mt-4 group-hover:text-primary transition-colors leading-tight ${isLarge ? 'text-3xl md:text-5xl' : 'text-xl md:text-3xl'}`}>
                            {article.title}
                        </h2>
                        {article.description && (
                            <div
